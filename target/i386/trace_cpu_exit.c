...
void trace_cpu_print_cpu_stats(int cpu_id) {
    ...
    fprintf(stderr, "VCPU %d exit stats\n", cpu_id);
    fprintf(stderr, "==================\n");
    fprintf(stderr, "TYPE COUNT MIN MAX TOTAL AVG\n");
    while (exit_iter) {
        exit_stats = g_hash_table_lookup(table, GINT_TO_POINTER(exit_iter->data));
        fprintf(stderr, "%d %d %.6fs %.6fs %.6fs %.6fs\n",
...
static void trace_print_dev_access_stats(int cpu_id, GHashTable** tables, char* dev_access_type) {
    ...
    fprintf(stderr, "VCPU %d %s stats\n", cpu_id, dev_access_type);
    fprintf(stderr, "==================\n");
    fprintf(stderr, "TYPE COUNT READ WRITE MIN MAX TOTAL AVG\n");
    while (dev_iter) {
        access_stats = g_hash_table_lookup(table, dev_iter->data);
        fprintf(stderr, "%s %d %d %d %.6fs %.6fs %.6fs %.6fs\n",
...
void trace_cpu_print_stats(void) {
    ...
    // mtree_info(fprintf, stderr, false);
}
...
static void trace_device_access_event(
    ...
    if (l!=len) {
        fprintf(stderr, "access length %d != %ld\n", len, (long)l);
    }
