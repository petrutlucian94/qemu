...
static void whpx_put_apic_state(APICCommonState *s, struct whpx_lapic_state *kapic)
{
    int i;
...
static void whpx_get_apic_state(APICCommonState *s, struct whpx_lapic_state *kapic)
{
    int i, v;
...
static void whpx_apic_put(CPUState *cs, run_on_cpu_data data)
{
    ...
    hr = whp_dispatch.WHvSetVirtualProcessorInterruptControllerState(
        whpx_global.partition,
        cs->cpu_index,
        &kapic,
        sizeof(kapic));
    if (FAILED(hr)) {
        fprintf(stderr, "WHvSetVirtualProcessorInterruptControllerState failed: %08lx\n", hr);
        abort();
    }
}
...
void whpx_apic_get(DeviceState *dev)
{
    ...
    HRESULT hr = whp_dispatch.WHvGetVirtualProcessorInterruptControllerState(
        whpx_global.partition,
        cpu->cpu_index,
        &kapic,
        sizeof(kapic),
...
static void whpx_send_msi(MSIMessage *msg)
{
    ...
    WHV_INTERRUPT_CONTROL interrupt = {
        .Type = delivery, // Values correspond to delivery modes
    ...
    if (FAILED(hr)) {
        fprintf(stderr, "whpx: injection failed, MSI (%llx, %x) lost (%08lx)\n", addr, data, hr);
    }
}
